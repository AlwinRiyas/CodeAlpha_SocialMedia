...
export async function createComment(userId, postId, payload = {}, io) {
  ...
  await createNotification({ type: 'COMMENT', recipientId: post.authorId, actorId: userId, postId, io })
  return comment
}
export function listComments(postId) { return prisma.comment.findMany({ where: { postId }, orderBy: { createdAt: 'asc' }, include: { author: { select: { id: true, username: true, displayName: true, avatarUrl: true } } } }) }
