...
export function validateRegistration(body = {}) {
  ...
  if (!USERNAME.test(String(username ?? ''))) {
    throw new AppError('Username must be 3-30 characters and contain only letters, numbers, or underscores', {
      statusCode: 400,
      code: 'VALIDATION_ERROR',
      expose: true,
    })
  ...
  if (typeof displayName !== 'string' || !displayName.trim() || displayName.trim().length > 80) {
    throw new AppError('Display name must be between 1 and 80 characters', {
      statusCode: 400,
      code: 'VALIDATION_ERROR',
      expose: true,
    })
