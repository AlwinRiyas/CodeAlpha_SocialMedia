...
export async function toggleLike(userId, postId, io) {
  ...
  if (existing) { await prisma.like.delete({ where: { userId_postId: { userId, postId } } }); return { liked: false } }
  await prisma.like.create({ data: { userId, postId } })
  await createNotification({ type: 'LIKE', recipientId: post.authorId, actorId: userId, postId, io })
  return { liked: true }
}
